...
# def get_credentials():
#     try:
#         # Try loading existing credentials from token.pkl
#         credentials = pickle.load(open("token.pkl", "rb"))
#     except (FileNotFoundError, EOFError):
#         # If token.pkl doesn't exist or is empty, initiate OAuth 2.0 flow
...
#         if username:
#             # Query the database for credentials associated with the username
#             cursor.execute("SELECT * FROM user_credentials WHERE username = %s LIMIT 1", (username,))
#             row = cursor.fetchone()
...
#             else:
#                 # Initiate OAuth 2.0 flow if credentials not found
#                 flow = InstalledAppFlow.from_client_secrets_file("client_secret.json", scopes=scopes)
#                 credentials = flow.run_local_server(port=0)
#                 print(credentials)
#                 # Save new credentials to the database
#                 access_token = credentials.token  # Access token
#                 token_expiry = credentials.expiry
#                 print(access_token)
#                 print(token_expiry)
#                 cursor.execute("INSERT INTO user_credentials (username, access_token,token_expiry) VALUES (%s, %s, %s)",
#                                (username, access_token, token_expiry))
#                 cnx.commit()
...
def insert_event():
    try:
        ...
        # Call the function to insert the event into Google Calendar and data into the database
        response = insert_event_and_data(data)
        return response
    except Exception as e:
        return f'Error inserting event and data: {str(e)}'


def insert_event_and_data(data):
    try:
        ...
        # Combine date and time to form datetime strings with seconds
        start_datetime_iso = f"{date}T{start_time}:00"
        end_datetime_iso = f"{date}T{end_time}:00"
        ...
        event = {
            ...
            'start': {
                ...
                'timeZone': 'Asia/Kolkata',  # Set timezone explicitly to Asia/Kolkata
            },
            'end': {
            ...
            'reminders': {
                ...
                'overrides': [
                    ...
                    {'method': 'email', 'minutes': 3}   # 1 minute before the event
                ]
            }
        }

        # Insert the event into the Google Calendar and data into the database
        ...
        if inserted_event:
            ...
            username= session.get('username')
            subject="Remainder!"
            message="Be Ready! "+username+", your task is scheduled on "+date+" at "+start_time
        ...
        return 'Failed to insert event into Google Calendar.'
    except Exception as e:
        return f'Error inserting event: {str(e)}'
